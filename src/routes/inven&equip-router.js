...
router.put("/equip/:charKey", authMW, async (req, res, next) => {
  try {
    ...
    // [검사 01] : 입력값이 유효한지
    if (!itemCode) {
      return res.status(400).json({ message: "요청된 아이템이 없슈!!" });
    }
    // [검사 02] : 여러 아이템이 입력됐는지
    if (Array.isArray(req.body)) {
      return res.status(400).json({ message: "장착은 하나씩만 가능합니다!!" });
    }
    // [검사 03] : 존재하는 캐릭터인지 확인
    const checkChar = await prisma.characters.findFirst({
      where: { charKey: +charKey },
    ...
    // [검사 04] : 내 계정의 캐릭터인지 확인
    if (checkChar.accountKey !== accountKey) {
      return res.status(403).json({ message: "당신 캐릭터가 아녀요!!" });
    }
    // [검사 05] : 내 캐릭의 인벤에 있는 아이템인지 확인
